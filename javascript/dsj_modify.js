...
if (!$.isNode()) {
    ...
    //var cks = accounts.map(x => getHeader(x));
    //dsj_header = cks.join('@')
	dsjCookie = $.getdata('dsjheader') ? $.getdata('dsjheader') : ""
	dsj_header = getHeader(dsjCookie)
}
//else
//    dsj_header = $.getdata('dsj_header') ? $.getdata('dsj_header') : ""

let dsj_headerArr = []
let dsj_headers = ""

//const dianshijia_API = 'mydianshijia.com/api'
const dianshijia_API = '47.95.69.248/api'
...
function getHeader(account) {
	...
	var accountCK = account.split('&');
	const headers = { ...defaultHeader, ...{ authorization: accountCK[1], userid: accountCK[0] }, ...account.headerEx }
    //const headers = { ...defaultHeader, ...{ authorization: accountCK[1], userid: accountCK[0] } }
...
!(async () => {
    ...
    for (let k = 0; k < dsj_headerArr.length; k++) {
        $.message = ""
        current_dsj_header = dsj_headerArr[k]
        current_reward_id = rewardId
        if (accounts && accounts[k].rewardId) {
            ...
            try {
                if (result.data.tempCoin) {
                    for (i = 0; i < result.data.tempCoin.length; i++) {
                        ...
                        $.get({
                            url: `http://api.47.95.69.248/api/coin/temp/exchange?id=` + coinid,
                            headers: DSJ_headers
                        }, (error, response, data))
                    }
                }
...
function dotask(code) {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v4/task/complete?code=${code}`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            //console.log(data)
...
function dsj_rwzt() {
    return new Promise((resolve) => {
        let url = {
            url: `http://47.95.69.248/api/v5/task/get`,
            headers: JSON.parse(current_dsj_header),
        }
                    ...
                    for (let index = 0; index < data.data.length; index++) {
                        ...
                        switch (task.code) {
                            ...
                            case 'playTask':
                                ...
                                break;
                            case 'ShortvideoPlay':
                                ShortvideoPlay = finish;
                                break;
                            case 'task_mobile_visit_album':
                                task_mobile_visit_album = finish;
                                break;
                            case 'task_mobile_upload_album':
                                task_mobile_upload_album = finish;
                                break;
                            case 'task_mobile_create_family':
                                task_mobile_create_family = finish;
                                break;
                            case '1M005':
                                M005 = finish;
                                break;
                        }
                        console.log(`${task.name}: ${finish ? '已完成' : '未完成'}`)
                    }
                    //浏览广告赚
...
function video() {
    return new Promise((resolve, reject) => {
        let url = {
            url: 'http:/47.95.69.248/api/v5/task/complete?code=ShortvideoPlay&comType=0',
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, async (err, resp, data) => {
            try {
...
function signin() {
    return new Promise((resolve, reject) => {
        let url = {
            url: 'http://47.95.69.248/api/v5/sign/signin?accelerate=0&ext=0&ticket=',
            headers: JSON.parse(current_dsj_header),
        }
...
function dsj_led() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://47.95.69.248/api/sign/chooseAdditionalReward?rewardId=${current_reward_id}`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            //console.log(data)
...
function sleep() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://47.95.69.248/api/taskext/getSleep?ext=0`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            try {
...
function CarveUp() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v2/taskext/getCarveUp?ext=1`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            $.log(`瓜分百万金币: ${data}`)
...
function getCUpcoin() {
    return new Promise((resolve, reject) => {
        $.get({
            url: `http://${dianshijia_API}/taskext/getCoin?ext=0&code=carveUp`,
            headers: JSON.parse(current_dsj_header),
        }, (error, response, data) => {
            //console.log(data)
...
function walk() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/taskext/getWalk?step=${walkstep}`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            $.log(`走路任务: ${data}\n`)
            ...
            if (result.data.unGetCoin > 10) {
                $.get({
                    url: `http://${dianshijia_API}/taskext/getCoin?code=walk&coin=${result.data.unGetCoin}&ext=1`,
                    headers: JSON.parse(current_dsj_header),
                }, (error, response, data) => { })
            }
            resolve()
...
function wakeup() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/taskext/getCoin?code=sleep&coin=1910&ext=1`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            $.log(`睡觉打卡: ${data}\n`)
...
function dsj_ggz() {
    return new Promise((resolve, reject) => {
        $.get({
            url: `http://${dianshijia_API}/v5/task/complete?code=H5Page_4&comType=0`,
            headers: JSON.parse(current_dsj_header),
...
function dsj_jrydz() {
    return new Promise((resolve, reject) => {
        $.get({
            url: `http://${dianshijia_API}/v5/task/complete?code=playTask&comType=0`,
            headers: JSON.parse(current_dsj_header),
        }, async (error, response, data) => {
            //console.log(data)
            //{"errCode":4,"msg":"不能重复签到"}
...
function dsj_sjbfx() {
    return new Promise((resolve, reject) => {
        $.get({
            url: `http://${dianshijia_API}/v5/task/complete?code=1M005&comType=0`,
            headers: JSON.parse(current_dsj_header),
        }, async (error, response, data) => {
            //console.log(data)
            //{"errCode":4,"msg":"不能重复签到"}
...
function dsj_dgt() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v5/task/complete?code=task_mobile_visit_song&comType=0`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            //console.log(data)
...
function dsj_fwxc() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v5/task/complete?code=task_mobile_visit_album&comType=0`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            //console.log(data)
...
function dsj_xcsds() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v5/task/complete?code=task_mobile_upload_album&comType=0`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            //console.log(data)
...
function dsj_kjth() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v5/task/complete?code=task_mobile_create_family&comType=0`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            //console.log(data)
...
function dsj_sdsp() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v5/task/complete?code=ShortvideoPlay&comType=0`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            //console.log(data)
...
function dsj_dqp(code) {
    return new Promise((resolve, reject) => {
        $.get({
            url: `http://${dianshijia_API}/coin/temp/exchange?id=${code}`,
            headers: JSON.parse(current_dsj_header),
        }, async (error, response, data) => {
            //console.log(data)
            let result = JSON.parse(data)
...
function getGametime() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v4/task/complete?code=gameTime&time=${gametimes}`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            $.log(`游戏时长: ${data}\n`)
...
function dsj_getinfo() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v3/user/info`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            let result = JSON.parse(data)
...
function dsj_info() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/activity/invite/bind?ename=${nickname}&eavatar=${headImgUrl}&uid=${JSON.parse(current_dsj_header).userid}&inviteCode=1126139`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            //console.log(data)
...
async function getdsj_header() {
    if ($request.url.match(/\/v3\/user\/info/)) {
        ...
        let bodyVal = { 'uuid': uuid, 'userid': userid, 'authorization': authorization, 'deviceId': deviceId, 'Host': '47.95.69.248', "appid": "0990028e54b2329f2dfb4e5aeea6d625" }
...
function cash() {
    return new Promise((resolve, reject) => {
        $.get({
            url: `http://${dianshijia_API}/cash/info`,
            headers: JSON.parse(current_dsj_header),
        }, (error, response, data) => {
            //if (logs) $.log(`现金: ${data}\n`)
...
function lhz() {
    return new Promise((resolve, reject) => {
        let url = {
            url: `http://${dianshijia_API}/v5/task/complete?code=H5Page_2&comType=0`,
            headers: JSON.parse(current_dsj_header),
        }
        $.get(url, (error, response, data) => {
            //console.log(data)
